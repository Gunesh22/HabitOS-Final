...
import { logEvent, pushEvents, pullEvents, performSnapshot } from './services/SyncEngine';
...
const HabitOSApp = () => {
  ...
  const deleteHabit = (e, id) => {
    e.stopPropagation();
    setHabitToDelete(id); // Opens confirmation modal
  };
  ...
  const confirmDeleteHabit = () => {
    if (habitToDelete) {
      ...
      logEvent('HABIT_DELETE', { id: habitToDelete });
  ...
  const syncData = React.useCallback(async () => {
    ...
    // Also push any pending local events
  ...
  const handleLogout = async () => {
    ...
    if (currentUser) {
      if (isOnline) {
        try {
          // Force a push before killing the session
          setIsSyncing(true);
          await pushEvents(currentUser.uid);
        } catch (e) {
          console.error("Logout auto-sync failed", e);
        } finally {
          setIsSyncing(false);
        }
      } else {
        ...
        const confirm = window.confirm(
          "⚠ OFFLINE WARNING ⚠\n\nYou are currently offline. Any unsaved changes will be LOST if you logout now.\n\nAre you sure you want to terminate the session?"
        );
        if (!confirm) return;
      }
    }

    // Clear local data to prevent "leaking" previous user's data
    localStorage.removeItem('habits-os-db');
    localStorage.removeItem('notes-os-db');
    localStorage.removeItem('habitos_sync_events');
    localStorage.removeItem('habitos_last_sync_time');
    localStorage.removeItem('habitos_pending_events_queue');
  ...
  // Auto-sync on changes (debounce in real app, here simple)
  useEffect(() => {
    const timer = setTimeout(() => {
      if (currentUser) pushEvents(currentUser.uid);
    }, 5000); // Push every 5s of inactivity
    return () => clearTimeout(timer);
  }, [habits, notes, currentUser]);
  ...
  const updateNote = (id, field, value) => {
    ...
    setNotes(updated);
    logEvent('NOTE_UPDATE', { id, [field]: value });
  };
  ...
  const toggleHistory = (habitId, dayIndex) => {
    ...
    // Find the new value 
    const isCompleted = updatedHabits.find(h => h.id === habitId).history[dayIndex];
    logEvent('HABIT_TOGGLE', { id: habitId, dayIndex, value: isCompleted });
  };
        ...
        <header style={{ textAlign: 'center', marginBottom: '40px', width: '100%', maxWidth: '800px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <div style={{ position: 'absolute', top: 20, right: 20, display: 'flex', gap: '10px', alignItems: 'center' }}>
            <span style={{
              fontSize: '12px',
              opacity: 0.6,
              color: !isOnline ? '#ff4444' : (isSyncing ? '#00ffcc' : 'inherit'),
              ...
              <div className="bento-grid">
                {habits.length === 0 && (
                  <div className="empty-state">
                    ...
                    <h3>System Idle</h3>
                    <p>Initialize a new protocol above to begin optimization.</p>
                  </div>
                )}
                {habits.map((habit) => {
                  const todayIdx = getTodayIndex();
              ...
              <div className="notes-grid">
                {notes.length === 0 && (
                  <div className="empty-state">
                    <div className="empty-icon">🧠</div>
                    <h3>Canvas Empty</h3>
                    <p>Create a new canvas to capture your ideations.</p>
                  </div>
                )}
                {notes.map(note => (
                  <SortableItem
